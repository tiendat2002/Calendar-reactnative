...
export default function EditScreen() : React.JSX.Element {
  ...
  const [selectedDay, setSelectedDay] = React.useState(format(new Date(), 'eeee, do MMMM, yyyy'));

  const onDayPress = (day : any) => {
    const formattedDate = day ? format(new Date(day.dateString), 'eeee, do MMMM, yyyy') : '';
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput style={styles.textHeader} placeholder="Nhập tiêu đề" value={title} onChangeText={newText => setTitle(newText)}/>
      <View style={styles.rowNote}>
        <View style={styles.iconNoteCover}>
          <MaterialIcons name="notes" size={30} />
        </View>
        <TextInput placeholder="Nhập ghi chú" style={styles.textNote} multiline={true} value={note} onChangeText={newText => setNote(newText)} />
      </View>
